...
export const postCache = {
  get: async <T>(postId: string | number, suffix?: string): Promise<T | null> => {
  ...
  delete: async (postId: string | number, suffix?: string): Promise<boolean> => {
    const key = generateCacheKey("POST", postId, suffix);
    return deleteCached(key);
  },
...
export const invalidateCache = {
  ...
  workspace: async (workspaceId: string): Promise<void> => {
    ...
    await deleteCachedByPattern(`${CACHE_KEYS.PAGE}:*:workspace:${workspaceId}`);
  },
